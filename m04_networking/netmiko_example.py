...
for device_type in [CSR, NXOS]:
    ...
    print(f"\n\n----- enabling privilege mode {device_type} -------------")
    connection.enable()
...
# CYCLE THROUGH DIFFERNT SHOW COMMANDS
print("\n\nBEGIN CYCLE THROUGH DIFFERNT SHOW COMMANDS")
...
for command_type, command in commands.items():
    ...
    # saving version for later parsing
    if command_type == SHOW_VERSION:
        csr_version_raw = csr_output
        nxos_version_raw = nxos_output

csr_connection.discnnect()
nxos_connection.discnnect()

# Now the harder part - parsing the ouput into some 'normalized" format
