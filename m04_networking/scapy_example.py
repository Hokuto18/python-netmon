...
# print("\n----- Packet sumaries --------------------")
# capture = scapy.sniff(iface='Wi-Fi', count=10)
# print(capture.nsummary())
#
# # CAPTURE DNS AND PRINT PACKETS
# print("\n----- DNS packet sumaries (collect 10 DNS packets) -----------------")
# capture = scapy.sniff(iface='Wi-Fi', filter="udp port 53", count=10)
# print(capture.nsummary())
#
...
#
#
# scapy.sniff(iface='Wi-Fi', prn=print_packet, filter='tcp port https', count=10)
#

# # CAPTURE AND HANDLE PACKETS AS THEY ARRIVE Using LAMBDA
# print("\n\n------ Capture and print packets as sniffed (using lambda) -----------------")
# scapy.sniff(iface='Wi-Fi', prn=lambda pkt: print(f"lambda {pkt.summary()}", filter="tcp port https", count=10))

# DISCOVER HOSTS ON NETWORK USING MANUAL ARP PING
# print("\n\n---- Discovery hosts on network using manual ARP ping -------------")
# ans, unans = scapy.srp(Ether(dst="ff:ff:ff:ff:ff:ff")/ARP(pdst="50.50.50.0/24"), timeout=2)
# ans.summary()
#
# # DISCOVER HOSTS ON NETWORK USING ARPING FUNCTION
# print("\n\n----- Discover hosts on network using arping() function -------------")
# ans, unans = scapy.arping("50.50.50.0/24")
# ans.summary()
#
# for res in ans.res:
...
# ans, unans = scapy.sr(IP(dst="50.50.50.1-250")/ICMP())
# ans.summary()
...
while True:
    ...
    ip = input("IP address on wich to scan ports: ")
    if not ip:
        print("\n----- Ending port scanning")
        break

    # answers, unans = scapy.sr(IP(dst="50.50.50.1")/TCP(flags="5", sport=666, dport=[22, 80, 21, 443]), timeout=1)
    ...
    print("/n----- Open/Closed port totals --------------------")
    print(f"\tOpen ports: {len(answers)}")
    print(f"\tlosed ports: {len(unans)}")
